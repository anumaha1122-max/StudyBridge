...
export default function MockTestResultScreen({ navigation, route }) {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <AppHeader title="Mock Test Result" navigation={navigation} />

      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          <Text style={styles.heroTitle}>Your Score: {score}/{total}</Text>
          <Text style={styles.heroSub}>Percentage: {percentage}%</Text>
        </View>

        <SCard
          title={percentage >= 70 ? "Good Job" : "Keep Practicing"}
          subtitle={
            percentage >= 70
              ? "You are doing well. Continue revision."
              : "Revise weak topics and try again."
          }
          status={percentage >= 70 ? "PASSED" : "PRACTICE"}
          icon="trophy-outline"
        />

        <AppButton title="Try Again" onPress={() => navigation.navigate("MockTest")} />
        <AppButton title="Go to Revision" variant="outline" onPress={() => navigation.navigate("Revision")} />
      </ScrollView>
    </SafeAreaView>
  );
