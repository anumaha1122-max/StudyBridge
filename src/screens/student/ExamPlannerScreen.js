...
export default function ExamPlannerScreen({ navigation, route }) {
  ...
  const exams = app.exams.filter((e) => e.classId === student?.classId || e.classId === 1);

  return (
    <SafeAreaView style={styles.safe}>
      <AppHeader title="Exam Planner" navigation={navigation} />

      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          <Text style={styles.heroTitle}>Upcoming Exams</Text>
          <Text style={styles.heroSub}>View exam date, syllabus, and instructions.</Text>
        </View>

        {exams.length === 0 ? (
          <SCard title="No exams" subtitle="Teacher-created exams will appear here." icon="calendar-outline" />
        ) : (
          exams.map((exam) => (
            <SCard
              key={exam.id}
              title={exam.title}
              subtitle={exam.subject + " • " + exam.examDate + " • " + exam.startTime}
              status="UPCOMING"
