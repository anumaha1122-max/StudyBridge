...
export default function StudentAchievementsScreen({ navigation, route }) {
  const app = useApp();
  const { currentUser, logout } = useAuth();
  const studentId = currentUser?.studentId || 1;
  const [success, setSuccess] = useState("");
  
  const achievements = app.achievements.filter((a) => a.studentId === studentId);
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          <Text style={styles.heroTitle}>My Achievements</Text>
          <Text style={styles.heroSub}>Badges and awards from teacher/admin.</Text>
        </View>

        {achievements.length === 0 ? (
          <SCard title="No achievements" subtitle="Awards will appear here." icon="ribbon-outline" />
        ) : (
          achievements.map((a) => (
            <SCard
              key={a.id}
              title={a.title}
              subtitle={a.description + " • Awarded by: " + a.awardedBy}
              status="AWARDED"
              icon="ribbon-outline"
            />
          ))
