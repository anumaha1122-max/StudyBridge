...
export default function StudentHomeworkScreen({ navigation, route }) {
  const app = useApp();
  ...
  const homework = app.homework.filter((h) => h.classId === student?.classId || h.classId === 1);

  const getStatus = (homeworkId) => {
    if (app.getHomeworkStatus) return app.getHomeworkStatus(homeworkId, studentId);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <AppHeader title="My Homework" navigation={navigation} />

      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          <Text style={styles.heroTitle}>Homework</Text>
          <Text style={styles.heroSub}>Open homework, read instructions, and submit your work.</Text>
        </View>

        {homework.length === 0 ? (
          <SCard title="No homework" subtitle="Assigned homework will appear here." icon="book-outline" />
        ) : (
          homework.map((item) => (
            <SCard
              key={item.id}
              title={item.title}
              subtitle={item.subject + " • Due: " + item.dueDate + " • " + item.teacherName}
