...
export default function DailyTasksScreen({ navigation, route }) {
  ...
  const tasks = app.tasks.filter((t) => t.studentId === studentId);

  const update = (taskId, status) => {
    app.updateTaskStatus(taskId, status);
    setSuccess("Task marked as " + status);
  };

  return (
    <SafeAreaView style={styles.safe}>
      <AppHeader title="Daily Tasks" navigation={navigation} />

      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          <Text style={styles.heroTitle}>My Tasks</Text>
          <Text style={styles.heroSub}>Teacher assigned tasks and your own tasks appear here.</Text>
        </View>

        <AppButton title="Add My Task" onPress={() => navigation.navigate("AddTask")} />

        {tasks.length === 0 ? (
          <SCard title="No tasks" subtitle="Tasks will appear here." icon="checkbox-outline" />
        ) : (
          tasks.map((task) => (
            <SCard
              key={task.id}
            ...
            >
              {task.status !== "COMPLETED" ? (
                <AppButton title="Mark Completed" onPress={() => update(task.id, "COMPLETED")} />
              ) : null}
            </SCard>
          ))
        )}
