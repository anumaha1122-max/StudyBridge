...
export default function StudentChatScreen({ navigation, route }) {
  const app = useApp();
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <AppHeader title="Student Chat" navigation={navigation} />

      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          <Text style={styles.heroTitle}>Message Teacher</Text>
          <Text style={styles.heroSub}>Send quick messages to teacher.</Text>
        </View>

        {app.messages
          .filter((m) => m.senderRole === "STUDENT" || m.receiverRole === "STUDENT")
          .map((m) => (
            <SCard
              key={m.id}
