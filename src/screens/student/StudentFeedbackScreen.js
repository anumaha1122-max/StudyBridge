...
export default function StudentFeedbackScreen({ navigation, route }) {
  const app = useApp();
  const { currentUser, logout } = useAuth();
  const studentId = currentUser?.studentId || 1;
  const [success, setSuccess] = useState("");
  
  const feedback = app.feedback.filter((f) => f.studentId === studentId);
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          <Text style={styles.heroTitle}>Teacher Feedback</Text>
          <Text style={styles.heroSub}>Academic and behavior feedback from teachers.</Text>
        </View>

        {feedback.length === 0 ? (
          <SCard title="No feedback" subtitle="Teacher feedback will appear here." icon="chatbox-outline" />
        ) : (
          feedback.map((f) => (
            <SCard
              key={f.id}
