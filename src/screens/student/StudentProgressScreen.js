...
export default function StudentProgressScreen({ navigation, route }) {
  ...
  const marks = app.marks.filter((m) => m.studentId === studentId);

  const average =
    marks.length === 0
      ? student?.performancePercentage || 0
      : Math.round(marks.reduce((sum, item) => sum + item.percentage, 0) / marks.length);

  return (
    <SafeAreaView style={styles.safe}>
      <AppHeader title="Progress" navigation={navigation} />

      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          <Text style={styles.heroTitle}>My Progress</Text>
          <Text style={styles.heroSub}>Marks, attendance, behavior and achievements.</Text>
        </View>
        ...
        <View style={styles.statGrid}>
          ...
          <View style={styles.statCard}>
            <Text style={styles.statValue}>{student?.behaviorScore || 0}</Text>
            <Text style={styles.statLabel}>Behavior</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statValue}>{app.achievements.filter((a) => a.studentId === studentId).length}</Text>
        ...
        {marks.length === 0 ? (
          <SCard title="No marks uploaded" subtitle="Teacher uploaded marks will appear here." icon="bar-chart-outline" />
        ) : (
          marks.map((m) => (
            <SCard
              key={m.id}
