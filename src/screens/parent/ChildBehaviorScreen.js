...
export default function ChildBehaviorScreen({ navigation, route }) {
  const app = useApp();
  ...
  const records = app.behavior.filter((b) => b.studentId === child?.id);

  return (
    <SafeAreaView style={styles.safe}>
      <AppHeader title="Child Behavior" navigation={navigation} />

      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          <Text style={styles.heroTitle}>Behavior Score: {child?.behaviorScore || 0}</Text>
          <Text style={styles.heroSub}>Positive and discipline records from teachers.</Text>
        </View>

        {records.length === 0 ? (
          <PCard title="No behavior records" subtitle="Teacher records will appear here." icon="star-outline" />
        ) : (
          records.map((b) => (
            <PCard
              key={b.id}
