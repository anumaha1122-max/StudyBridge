...
export default function ChildAttendanceScreen({ navigation, route }) {
  ...
  const records = app.attendance.filter((a) => a.studentId === child?.id);

  return (
    <SafeAreaView style={styles.safe}>
      <AppHeader title="Child Attendance" navigation={navigation} />

      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          <Text style={styles.heroTitle}>Attendance</Text>
          <Text style={styles.heroSub}>Daily attendance records and absent alerts.</Text>
        </View>
        ...
        {records.length === 0 ? (
          <PCard title="No attendance records" subtitle="Teacher attendance records will appear here." icon="calendar-outline" />
        ) : (
          records.map((item) => (
            <PCard
              key={item.id}
              title={item.studentName}
              subtitle={item.subject + " • " + item.date + " • " + (item.remark || "No remark")}
              status={item.status}
