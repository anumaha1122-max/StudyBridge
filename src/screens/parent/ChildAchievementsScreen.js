...
export default function ChildAchievementsScreen({ navigation, route }) {
  ...
  const records = app.achievements.filter((a) => a.studentId === child?.id);

  return (
    <SafeAreaView style={styles.safe}>
      <AppHeader title="Child Achievements" navigation={navigation} />

      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          <Text style={styles.heroTitle}>Achievements</Text>
          <Text style={styles.heroSub}>Badges and awards given by teachers/admin.</Text>
        </View>

        {records.length === 0 ? (
          <PCard title="No achievements" subtitle="Achievements will appear here." icon="ribbon-outline" />
        ) : (
          records.map((a) => (
            <PCard
              key={a.id}
