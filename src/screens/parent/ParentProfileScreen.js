...
export default function ParentProfileScreen({ navigation, route }) {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <AppHeader title="Parent Profile" navigation={navigation} />

      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          <Text style={styles.heroTitle}>{parent?.name || currentUser?.name}</Text>
          <Text style={styles.heroSub}>{currentUser?.email || "parent@studybridge.com"}</Text>
        </View>

        <PCard
          title="Linked Child"
          subtitle={(child?.name || "Student") + " • " + (child?.className || "") + " • Roll: " + (child?.rollNumber || "")}
          status="LINKED"
          icon="person-outline"
        />
        ...
        <PCard
          ...
          subtitle={"Role: " + (currentUser?.role || "PARENT")}
          status="ACTIVE"
