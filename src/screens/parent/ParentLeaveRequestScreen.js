...
export default function ParentLeaveRequestScreen({ navigation, route }) {
  const app = useApp();
  ...
  const leaves = app.leaveRequests.filter((l) => l.parentId === parentId || l.studentId === child?.id);
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          ...
          <Text style={styles.heroSub}>Send leave request to teacher and track approval.</Text>
        </View>

        <View style={styles.form}>
          <AppInput label="Reason" value={form.reason} onChangeText={(v) => setForm({ ...form, reason: v })} />
          <AppInput label="From Date" value={form.fromDate} onChangeText={(v) => setForm({ ...form, fromDate: v })} placeholder="2026-06-20" />
          <AppInput label="To Date" value={form.toDate} onChangeText={(v) => setForm({ ...form, toDate: v })} placeholder="2026-06-21" />
        ...
        {leaves.length === 0 ? (
          <PCard title="No leave requests" subtitle="Submitted requests will appear here." icon="mail-outline" />
        ) : (
          leaves.map((l) => (
            <PCard
              key={l.id}
