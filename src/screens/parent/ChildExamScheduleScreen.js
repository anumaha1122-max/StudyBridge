...
export default function ChildExamScheduleScreen({ navigation, route }) {
  const app = useApp();
  ...
  const exams = app.exams.filter((e) => e.classId === child?.classId || e.classId === 1);

  return (
    <SafeAreaView style={styles.safe}>
      <AppHeader title="Exam Schedule" navigation={navigation} />

      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          <Text style={styles.heroTitle}>Child Exam Schedule</Text>
          <Text style={styles.heroSub}>Upcoming exam date, time, syllabus and instructions.</Text>
        </View>

        {exams.length === 0 ? (
          <PCard title="No exams" subtitle="Teacher created exams will appear here." icon="calendar-outline" />
        ) : (
          exams.map((exam) => (
            <PCard
              key={exam.id}
              title={exam.title}
              subtitle={exam.subject + " • " + exam.examDate + " • " + exam.startTime + " • " + exam.syllabus}
              status="UPCOMING"
