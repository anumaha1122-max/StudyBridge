...
export default function ParentNotificationsScreen({ navigation, route }) {
  ...
  const read = (id) => {
    if (app.markNotificationRead) app.markNotificationRead(id);
  };
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          <Text style={styles.heroTitle}>Parent Alerts</Text>
          <Text style={styles.heroSub}>Homework, marks, attendance, fees, diary and meeting alerts.</Text>
        </View>

        {notifications.length === 0 ? (
          <PCard title="No notifications" subtitle="Parent updates will appear here." icon="notifications-outline" />
        ) : (
          notifications.map((n) => (
            <PCard
              key={n.id}
            ...
            >
              {!n.read ? <AppButton title="Mark Read" onPress={() => read(n.id)} /> : null}
            </PCard>
          ))
        )}
      </ScrollView>
