...
export default function ChildHomeworkScreen({ navigation, route }) {
  const app = useApp();
  ...
  const homework = app.homework.filter((h) => h.classId === child?.classId || h.classId === 1);

  const getSubmission = (homeworkId) =>
    app.homeworkSubmissions.find(
      (s) => s.homeworkId === homeworkId && s.studentId === child?.id
    );

  return (
    <SafeAreaView style={styles.safe}>
      <AppHeader title="Child Homework" navigation={navigation} />

      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          <Text style={styles.heroTitle}>Homework Tracking</Text>
          <Text style={styles.heroSub}>Track assigned, submitted and reviewed homework.</Text>
        </View>

        {homework.length === 0 ? (
          <PCard title="No homework" subtitle="Teacher assigned homework will appear here." icon="book-outline" />
        ) : (
          homework.map((item) => {
