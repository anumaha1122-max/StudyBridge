...
export default function ChildTimetableScreen({ navigation, route }) {
  const app = useApp();
  ...
  const periods = app.timetable.filter((t) => t.classId === child?.classId || t.classId === 1);

  return (
    <SafeAreaView style={styles.safe}>
      <AppHeader title="Child Timetable" navigation={navigation} />

      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          <Text style={styles.heroTitle}>Class Timetable</Text>
          <Text style={styles.heroSub}>Daily periods, subjects and teachers.</Text>
        </View>
        ...
        {periods.map((p) => (
          <PCard
            key={p.id}
            title={p.subject}
            subtitle={p.day + " • Period " + p.period + " • " + p.time + " • " + p.teacher}
            status={p.room || "ROOM"}
            icon="time-outline"
          />
        ))}
