...
export default function ParentFeedbackScreen({ navigation, route }) {
  const app = useApp();
  ...
  const feedback = app.feedback.filter((f) => f.studentId === child?.id);

  return (
    <SafeAreaView style={styles.safe}>
      <AppHeader title="Teacher Feedback" navigation={navigation} />

      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          <Text style={styles.heroTitle}>Child Feedback</Text>
          <Text style={styles.heroSub}>Academic and behavior feedback from teachers.</Text>
        </View>

        {feedback.length === 0 ? (
          <PCard title="No feedback" subtitle="Teacher feedback will appear here." icon="chatbox-outline" />
        ) : (
          feedback.map((f) => (
            <PCard
              key={f.id}
              title={f.teacherName + " • " + f.subject}
              subtitle={f.message}
              status={f.type}
