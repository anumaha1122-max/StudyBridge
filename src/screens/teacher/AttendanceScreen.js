...
export default function AttendanceScreen({ navigation, route }) {
  ...
  const submit = () => {
    ...
    setSuccess("Attendance saved. Student and parent notified.");
  };
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          ...
          <Text style={styles.heroSub}>Absent alerts will be sent to parents.</Text>
        </View>
        ...
        <View style={styles.form}>
          ...
          <PickerRow label="Status">
            {["PRESENT", "ABSENT", "LATE", "LEAVE"].map((s) => (
              <Chip key={s} title={s} active={form.status === s} onPress={() => setForm({ ...form, status: s })} />
            ))}
          </PickerRow>
        ...
        {app.attendance.map((a) => (
          <TCard
            key={a.id}
            title={a.studentName}
            subtitle={a.subject + " • " + a.date + " • " + (a.remark || "No remark")}
            status={a.status}
            icon="calendar-outline"
          />
        ))}

        <SuccessModal visible={!!success} title="Attendance" message={success} onClose={() => setSuccess("")} />
      </ScrollView>
    </SafeAreaView>
  );
