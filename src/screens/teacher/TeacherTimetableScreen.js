...
export default function TeacherTimetableScreen({ navigation, route }) {
  const app = useApp();
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          ...
          <Text style={styles.heroSub}>Class timetable created by admin.</Text>
        </View>
        ...
        {app.timetable.map((item) => (
          <TCard
            key={item.id}
            title={item.subject}
            subtitle={item.day + " • Period " + item.period + " • " + item.time + " • Room " + (item.room || "")}
            status={item.teacher}
            icon="time-outline"
          />
        ))}
