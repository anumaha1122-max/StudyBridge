...
export default function TeacherClassesScreen({ navigation, route }) {
  const app = useApp();
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <AppHeader title="My Classes" navigation={navigation} />

      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          <Text style={styles.heroTitle}>Assigned Classes</Text>
          <Text style={styles.heroSub}>View classes and open student list.</Text>
        </View>

        {app.classes.map((item) => {
          const count = app.students.filter((s) => s.classId === item.id).length;

          return (
            <TCard
              key={item.id}
              title={item.name || item.className + " " + item.section}
              subtitle={"Academic Year: " + item.academicYear + " • Students: " + count}
              status="ACTIVE"
              icon="school-outline"
