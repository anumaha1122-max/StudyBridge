...
export default function TeacherBehaviorScreen({ navigation, route }) {
  const app = useApp();
  ...
  const submit = () => {
    ...
    setForm({ ...form, remark: "" });
    setSuccess("Behavior record added. Parent notified.");
  };

  return (
    <SafeAreaView style={styles.safe}>
      <AppHeader title="Behavior" navigation={navigation} />

      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          <Text style={styles.heroTitle}>Behavior Tracker</Text>
          <Text style={styles.heroSub}>Add positive or discipline behavior records.</Text>
        </View>
        ...
        <View style={styles.form}>
          ...
          <PickerRow label="Type">
            {["POSITIVE", "DISCIPLINE"].map((s) => (
              <Chip key={s} title={s} active={form.type === s} onPress={() => setForm({ ...form, type: s, points: s === "POSITIVE" ? "5" : "-5" })} />
            ))}
          </PickerRow>
          ...
          <AppButton title="Save Behavior" onPress={submit} />
        </View>
