...
export default function StudyMaterialProgressScreen({ navigation, route }) {
  const app = useApp();
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <AppHeader title="Material Progress" navigation={navigation} />

      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          <Text style={styles.heroTitle}>Notes Read Progress</Text>
          <Text style={styles.heroSub}>Track which students opened uploaded study material.</Text>
        </View>
        ...
        {app.notes.map((note) => (
          <TCard
            key={note.id}
            title={note.title}
            subtitle={note.subject + " • Type: " + note.type}
            status={(note.readBy?.length || 0) + " READ"}
