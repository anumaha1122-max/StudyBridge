...
export default function TeacherProfileScreen({ navigation, route }) {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <AppHeader title="Teacher Profile" navigation={navigation} />

      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          <Text style={styles.heroTitle}>{currentUser?.name || "Teacher"}</Text>
          <Text style={styles.heroSub}>{currentUser?.email || "teacher@studybridge.com"}</Text>
        </View>

        <TCard
          title="Teacher Account"
          subtitle={"Role: " + (currentUser?.role || "TEACHER") + " • Class ID: " + (currentUser?.classId || 1)}
          status="ACTIVE"
        ...
        <TCard
          ...
          subtitle="Homework, attendance, marks, notes, doubts, parent meetings and diary."
          status="FULL ACCESS"
