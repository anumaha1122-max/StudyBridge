...
export default function CreateTaskScreen({ navigation, route }) {
  ...
  const submit = () => {
    if (!form.title || !form.dueDate) {
      setSuccess("Please enter task title and due date.");
      return;
    }

    app.addTask(form);
    setForm({ ...form, title: "", dueDate: "" });
    setSuccess("Task created for student.");
  };

  return (
    <SafeAreaView style={styles.safe}>
      <AppHeader title="Create Task" navigation={navigation} />

      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          <Text style={styles.heroTitle}>Student Task</Text>
          <Text style={styles.heroSub}>Assign daily study tasks to selected student.</Text>
        </View>
        ...
        <View style={styles.form}>
          ...
          <AppInput label="Task Title" value={form.title} onChangeText={(v) => setForm({ ...form, title: v })} />
          <AppInput label="Subject" value={form.subject} onChangeText={(v) => setForm({ ...form, subject: v })} />

          <PickerRow label="Priority">
            {["LOW", "MEDIUM", "HIGH"].map((s) => (
              <Chip key={s} title={s} active={form.priority === s} onPress={() => setForm({ ...form, priority: s })} />
            ))}
          </PickerRow>

          <AppInput label="Due Date" value={form.dueDate} onChangeText={(v) => setForm({ ...form, dueDate: v })} />

          <AppButton title="Create Task" onPress={submit} />
        </View>
        ...
        <SuccessModal visible={!!success} title="Task" message={success} onClose={() => setSuccess("")} />
      </ScrollView>
    </SafeAreaView>
  );
