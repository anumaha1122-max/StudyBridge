...
export default function TeacherFeedbackScreen({ navigation, route }) {
  const app = useApp();
  ...
  const submit = () => {
    if (!form.message) {
      setSuccess("Please enter feedback message.");
      return;
    }
    ...
    setSuccess("Feedback sent to student and parent.");
  };
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          ...
          <Text style={styles.heroSub}>Academic and behavior feedback will reach student and parent.</Text>
        </View>
        ...
        <View style={styles.form}>
          ...
          <PickerRow label="Feedback Type">
            {["ACADEMIC", "BEHAVIOR", "IMPROVEMENT"].map((s) => (
              <Chip key={s} title={s} active={form.type === s} onPress={() => setForm({ ...form, type: s })} />
            ))}
          </PickerRow>
          ...
          <AppButton title="Send Feedback" onPress={submit} />
        </View>
        ...
        {app.feedback.map((f) => (
          <TCard
            key={f.id}
            title={f.studentName || "Student"}
            subtitle={f.subject + " • " + f.message}
            status={f.type}
            icon="chatbox-ellipses-outline"
          />
        ))}

        <SuccessModal visible={!!success} title="Feedback" message={success} onClose={() => setSuccess("")} />
      </ScrollView>
    </SafeAreaView>
  );
